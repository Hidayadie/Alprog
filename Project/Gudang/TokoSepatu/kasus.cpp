...
struct keranjang {
    int         index   ;   // Digunakan untuk nomor ID keranjang
...
keranjang *kepala   ,   // Node paling awal
          ...
          *iterasi  ;   // Dipakai untuk looping dan mengecek
...
// untuk hapus keranjang sepertinya lebih enak jika
// ada penomoran/nama untuk setiap node? Menggunakan
// iterator atau int index yang akan disimpan untuk
// setiap nodenya sendiri sehingga saat penghapusan
// dilakukan secara memilih index mana yang akan
// dihapus, contoh;
// Jika ada 5 keranjang, dan user membeli keranjang
// nomor 1,3 maka akan dilakukan penghapusan node
// 1 dan 3, Setelah itu, node akan mengurutkan kembali
// sesuai yang ada seperti semua
// 2 menjadi 1 | 4 menjadi 2 | 5 menjadi 3

// Dan juga mode untuk penghapusan semua node, yaitu
// saat user membeli semua barang yang ada dikeranjang

void _linked__kurang() {
    //if ()
}
...
void _linked__print() {
    ...
    if (kepala != NULL) {
        cout << "isi: ";
        while (iterasi != NULL) {
            cout << iterasi->data.Nama;
            if (iterasi->next != NULL) {
                cout << " <-> ";
            }
